...
main()
{
	...
	cp ${BINARIES_DIR}/rootfs.cpio.gz ${2}/fsl-image-core-ls1028ardb.cpio.gz
	cd ${2}/
	/usr/bin/mkimage -f kernel-ls1028a-rdb.its kernel-ls1028a-rdb.itb
	cd ${3}
	cp ${2}/kernel-ls1028a-rdb.itb ${BINARIES_DIR}/
	rm ${2}/fsl-image-core-ls1028ardb.cpio.gz
	...
	sed -e "s/%FILES%/${FILES}/" -e "s/%PARTITION_SIZE%/${BR2_ROOTFS_PARTITION_SIZE}/" \
		board/nxp/ls1028ardb/genimage.xspi-cfg.template > ${GENIMAGE_CFG}
