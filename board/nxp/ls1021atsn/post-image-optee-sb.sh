...
main()
{
	...
	mkimage -A arm -T script -a 0 -e 0x40 -d board/nxp/ls1021atsn/bootscript.txt.tee ${BUILD_DIR}/host-cst-${CST_VERSION}/bootscript
	...
	cp ${BINARIES_DIR}/ls1021a-tsn.dtb ${BUILD_DIR}/host-cst-${CST_VERSION}/uImage.dtb
	cp ${BINARIES_DIR}/kernel-ls1021a-tsn.itb ${BUILD_DIR}/host-cst-${CST_VERSION}/kernel.itb
	...
	local BOARDNAME="ls1021atsn-sb-optee"
	sed -e "s/%PLATFORM%/${BOARDNAME}/" board/nxp/common/version.json > ${BINARIES_DIR}/version.json

	local FILES=""kernel-ls1021a-tsn.itb", "version.json""
