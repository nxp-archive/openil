...
#define GET_QBV_STATUS_CMD_PREFIX "tsntool qbvget --device "
...
int get_qbv_status(xmlNodePtr node)
{
	...
	for(port=0; port<TOTAL_PORTS; port++){
		...
		oper_node = xmlNewChild(interface_node, NULL, BAD_CAST "oper", NULL);
		strcpy(cmd, GET_QBV_STATUS_CMD_PREFIX);
		strcat(cmd ,interface_name[port]);
		fp = popen(cmd, "r");
		temp = NULL;
		if(fp){
			while(fgets(cmd_rst, MAX_PORT_NAME_LEN, fp) != NULL){
				...
				if(!strncmp(cmd_rst, "   listmax", 10)){
					temp = extract_data(cmd_rst, "=");
					temp_ul = strtoul(temp, NULL, 16);
					sprintf(temp, "%ld", temp_ul);
					if(temp){
						xmlNewTextChild(interface_node, NULL, BAD_CAST "supported-list-max", BAD_CAST temp);
					}
				}else if(!strncmp(cmd_rst, "   cycletimeext", 15)){
					temp = extract_data(cmd_rst, "=");
					temp_ul = strtoul(temp, NULL, 16);
					sprintf(temp, "%ld", temp_ul);
					if(temp){
				...
				}else if(!strncmp(cmd_rst, "   configchangetime", 19)){
					temp = extract_data(cmd_rst, "=");
					
					if(temp){
						second = strtoul(temp, NULL, 16);
						nanosecond = second%1000000000;
						second = second/1000000000;
						time_node = xmlNewChild(interface_node, NULL, BAD_CAST "config-change-time", NULL);
				...
				}else if(!strncmp(cmd_rst, "   currenttime", 14)){
					temp = extract_data(cmd_rst, "=");
					...
					if(temp){
						...
						nanosecond = second%1000000000;
						second = second/1000000000;
						time_node = xmlNewChild(interface_node, NULL, BAD_CAST "current-time", NULL);
						sprintf(temp, "%ld", second);
						xmlNewTextChild(time_node, NULL, BAD_CAST "seconds", BAD_CAST temp);
						sprintf(temp, "%ld", nanosecond);
						xmlNewTextChild(time_node, NULL, BAD_CAST "fractional-seconds", BAD_CAST temp);
					}
				}else if(!strncmp(cmd_rst, "   cycletime", 12)){
					temp = extract_data(cmd_rst, "=");
					...
					if(temp){
						...
						nanosecond = second%1000000000;
						second = second/1000000000;
						time_node = xmlNewChild(oper_node, NULL, BAD_CAST "oper-cycle-time", NULL);
						sprintf(temp, "%ld", second);
						xmlNewTextChild(time_node, NULL, BAD_CAST "seconds", BAD_CAST temp);
						sprintf(temp, "%ld", nanosecond);
						xmlNewTextChild(time_node, NULL, BAD_CAST "fractional-seconds", BAD_CAST temp);
					}
				}else if(!strncmp(cmd_rst, "   configpending", 16)){
					xmlNewTextChild(interface_node, NULL, BAD_CAST "config-pending", BAD_CAST "true");
				}
			}
		}
